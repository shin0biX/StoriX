...
The Basic Flow
Users create an account and authenticate to access protected features.
Each user is associated with a subscription plan and its storage limits.
Files are stored on disk while their metadata and ownership information are stored in the database.
Storage limits and ownership checks are enforced during file operations.
Files can be kept private or made available for sharing.
🚀 Getting Started
Prerequisites
Python 3.9+
Node.js 16+
